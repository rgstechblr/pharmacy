...
<div class="alert alert-danger fade in" id="divNoWarehouse" style="display:none">
    <a href="#" class="close" data-dismiss="alert">&times;</a>
    <strong>Error!</strong> Warehouseno is alreadyexists
</div>
<style>
	.form-control {
	width:80%;
...
<button type="button" class="btn" id="btnSubmit" onclick="GetLocation()">Show Adress in Google Map</button> 
   ...
   <div class="col-xs-5 col-md-6">
        <div class="form-group">
        <input type="hidden" name="warehouseCheck" id="warehouseCheck">
...
<script type="text/javascript">
	function validate(formObj)
	{
		...
		else if(formObj.warehouseCheck.value=="" || formObj.warehouseCheck.value=="fail")
		{
			alert(" warehouseSNO already exists");
			$("#warehouseSNO").focus();
			return false;
		}
	...
	$('#warehouseSNO').keyup(function() {
   	$('#divNoWarehouse').css("display", "none"); 	
   
	
		
				var data = 'warehouseSNO='
					+ encodeURIComponent(this.value);
					
			$.ajax({
				url : "checkwarehousesno.htm",
				data : data,
				...
				success : function(response) {
					...
					$("#warehouseCheck").val(response.msg);
					if(response.msg=="fail")
					{
						$('#divNoWarehouse').css("display", "block"); 	
					}
					else
					{
						$('#divNoWarehouse').css("display", "none"); 	
